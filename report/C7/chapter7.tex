...
The main limitation on the constraint based learning approach is that it cannot handle multiple function calls. For example, if the tool encounters the equality constraint \lstinline{eq(add(f(x - 1), f(x - 2)), 8)}
